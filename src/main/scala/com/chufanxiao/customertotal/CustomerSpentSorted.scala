package com.chufanxiao.customertotal
...
object TotalSpentByCustomerSorted {
  ...
  def main(args: Array[String]) {
    ...
    val sortedTotalByCustomer = totalByCustomer.map(x => (x._2,x._1)).sortByKey(ascending = false)

    val results = sortedTotalByCustomer.collect()

    // Print the results.
    for (result <- results) {
      val total = result._1
      val customerId = result._2
      println(s"$customerId : $$ $total")
    }
  }
